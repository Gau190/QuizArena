# Deploy ExamHub Free / Low-Cost

## Khuyến nghị nhanh

Vì ExamHub dùng ASP.NET Core 8 + SQL Server, phương án ít phải sửa code nhất là Azure:
...
4. Database name: `ExamHub`.
5. Server auth: SQL authentication.
6. Ghi lại:
   - Server name, ví dụ: `examhub-sql.database.windows.net`
   - Login, ví dụ: `examhub_admin`
   - Password

Connection string mẫu:

```text
Server=tcp:examhub-sql.database.windows.net,1433;Initial Catalog=ExamHub;Persist Security Info=False;User ID=examhub_admin;Password=YOUR_PASSWORD;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;
```
...
dotnet tool run dotnet-ef database update \
  --project ExamHub.Infrastructure/ExamHub.Infrastructure.csproj \
  --startup-project ExamHub.Web/ExamHub.Web.csproj
```
...
Seed data sẽ tự chạy khi `ExamHub.Web` chạy ở `Development`. Với production, có thể tạm set `ASPNETCORE_ENVIRONMENT=Development` lần đầu để seed, sau đó đổi lại `Production`.
...
ConnectionStrings__Default=Server=tcp:examhub-sql.database.windows.net,1433;Initial Catalog=ExamHub;Persist Security Info=False;User ID=examhub_admin;Password=YOUR_PASSWORD;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;
...
```bash
dotnet publish ExamHub.Web/ExamHub.Web.csproj -c Release -o publish
```
...
Đây vẫn là phương án ít rủi ro kỹ thuật nhất cho ExamHub hiện tại.
...
Được, nếu đây là đồ án/demo/thi thử nhỏ. Với ExamHub hiện tại, 30 MB DB có thể đủ cho:
...
Render hỗ trợ web service từ Git repo hoặc Docker image. Với ExamHub, nên dùng Dockerfile trong repo.
