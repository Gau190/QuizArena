# QuizArena — Đặc tả kỹ thuật đầy đủ cho AI Agent
...
| Mục | Giá trị |
|-----|---------|
| Tên hệ thống | QuizArena — Website Thi Trắc Nghiệm Trực Tuyến |
...
```
QuizArena.sln
├── QuizArena.Web          # ASP.NET Core MVC (Views, Controllers)
├── QuizArena.Api          # ASP.NET Core Web API (endpoints cho JS client)
├── QuizArena.Core         # Domain entities, interfaces, enums
├── QuizArena.Infrastructure  # EF Core DbContext, Repositories, Migrations
├── QuizArena.Tests        # xUnit unit tests
└── docker-compose.yml
```
...
async function syncTimer() {
    const res = await fetch(`/api/v1/exam/time-remaining/${attemptId}`);
...
|---------|--------|------|-------|
| `/api/v1/auth/login` | POST | — | Trả JWT token |
| `/api/v1/auth/logout` | POST | Any | Xóa session |
| `/api/v1/exam/start/{examId}` | POST | Student | Tạo ExamAttempt, snapshot câu hỏi |
| `/api/v1/exam/question/{attemptId}/{index}` | GET | Student | Lấy câu hỏi theo thứ tự |
| `/api/v1/exam/answer` | POST | Student | Lưu đáp án (upsert) |
| `/api/v1/exam/submit/{attemptId}` | POST | Student | Nộp bài, chấm điểm |
| `/api/v1/exam/time-remaining/{attemptId}` | GET | Student | Server time còn lại |
| `/api/v1/exam/flag` | POST | Student | Ghi nhận vi phạm anti-cheat |
| `/api/v1/teacher/stats/{examId}` | GET | Teacher | Thống kê kết quả |
| `/api/v1/admin/users` | GET/POST/PUT/DELETE | Admin | CRUD users |
| `/api/v1/admin/subjects` | GET/POST/PUT/DELETE | Admin | CRUD subjects |
...
<!-- _Layout.cshtml -->
<nav class="navbar">
  <div class="nav-logo">QuizArena</div>
...
| # | Màn hình | Route | Role |
|---|----------|-------|------|
| 1 | Đăng nhập | `/dang-nhap` | Public |
| 2 | Quản lý người dùng | `/quan-tri/nguoi-dung` | Admin |
| 3 | Quản lý môn thi | `/quan-tri/mon-thi` | Admin |
| 4 | Ngân hàng câu hỏi | `/giang-vien/ngan-hang-cau-hoi` | Teacher |
| 5 | Tạo / sửa kỳ thi | `/giang-vien/ky-thi/tao` | Teacher |
| 6 | Thống kê kết quả | `/giang-vien/ket-qua/{examId}` | Teacher |
| 7 | Dashboard thí sinh | `/thi-sinh/tong-quan` | Student |
| 8 | Làm bài thi | `/thi-sinh/lam-bai/{attemptId}` | Student |
| 9 | Kết quả bài thi | `/thi-sinh/ket-qua/{attemptId}` | Student |
...
ConnectionStrings__Default=Server=...;Database=QuizArena;...
...
```
ConnectionStrings__Default=Server=localhost;Database=QuizArena;User Id=sa;Password=YourPwd;
...
WORKDIR /src
COPY *.sln .
COPY QuizArena.Core/*.csproj QuizArena.Core/
COPY QuizArena.Infrastructure/*.csproj QuizArena.Infrastructure/
COPY QuizArena.Web/*.csproj QuizArena.Web/
RUN dotnet restore
COPY . .
RUN dotnet publish QuizArena.Web -c Release -o /app/publish
...
services:
  web:
    ...
    ports:
      - "8080:8080"
    environment:
      - ConnectionStrings__Default=Server=db;Database=QuizArena;User Id=sa;Password=${DB_PASSWORD};TrustServerCertificate=True
...
```xml
<!-- QuizArena.Infrastructure.csproj -->
<PackageReference Include="Microsoft.EntityFrameworkCore.SqlServer" Version="8.*" />
<PackageReference Include="Microsoft.EntityFrameworkCore.Tools" Version="8.*" />
<PackageReference Include="BCrypt.Net-Next" Version="4.*" />

<!-- QuizArena.Web.csproj -->
...
2. **QuizArena.Core** — định nghĩa tất cả entities, enums, interface repositories.
3. **QuizArena.Infrastructure** — implement DbContext, migrations, SeedData, repositories.
