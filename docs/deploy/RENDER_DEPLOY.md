# Deploy QuizArena Web to Render
...
Vì vậy deploy Render chỉ cần chạy `QuizArena.Web` và trỏ đến Somee SQL Server.
...
2. New -> Web Service.
3. Connect GitHub repository chứa QuizArena.
4. Chọn:
   - Runtime: `Docker`
...
```text
workstation id=QuizArena.mssql.somee.com;packet size=4096;user id=<login>;pwd=<password>;data source=QuizArena.mssql.somee.com;persist security info=False;initial catalog=QuizArena;TrustServerCertificate=True
```
...
```text
https://<service-name>.onrender.com/dang-nhap
```
...
```text
quizarena-production-secret-please-change-2026
```
